...
public class Billing extends AbstractCommandHandler{
  ...
  private StringBuffer readFile(File billTemplate) throws FileNotFoundException, IOException {
	...
	private void fillBillTotal(String[] args, HashMap<String, String> subs) throws SQLException {
     ...
     private void fillPaymentTotal(String[] args, HashMap<String, String> subs) throws SQLException {
	...
	private void fillPurchasesSection(String[] args, HashMap<String, String> subs) throws ParseException,
	  SQLException {
	    ...
	    purchasesStmt.setInt(2, identifier);
  ...
  SQLException {
    ...
    SimpleDateFormat format = setupDates(args);
    subs.put("billDate", format.format(billDate.getTime()));
  ...
  private SimpleDateFormat setupDates(String[] args) throws ParseException {
    ...
    if(args.length > 3){  // We must have included a Date for billing
      startDate.setTime(format.parse(args[3]));
    } else {
  ...
  private void fillVendorInfo(HashMap<String, String> subs) throws SQLException {
    PreparedStatement customerInfoStmt = prepareStatement("SELECT name, address, phone FROM Vendor WHERE id = ?");
    customerInfoStmt.setInt(1, identifier);
    customerInfoStmt.execute();
    subs.put("invoiceNumber", identifier + "-" + System.currentTimeMillis());  //pseudo unique bill identifier
    ...
    if(subs.get("vendorName") == null){
      exitProgram("Vendor does not exist.\n Vendor Id: "+identifier);
    }
  }
  ...
  private void fillCustomerInfo(HashMap<String, String> subs) throws SQLException {
    ...
    if(subs.get("customerName") == null){
      exitProgram("Customer does not exist.\n Customer Id: "+identifier);
    }
  }
